  ...
  if( isset($_POST['user_register'])) {
    ...
    if($_POST['user'] == 1 ) { //teacher
        if(!($user_type_error && $name_error && $gender_error && $phone_error && $username_error && $email_error && $password_error && $pass_mathc_error && $dept_error && $phone_error && $address_error)) { 
            move_uploaded_file($tmp_name, $uplodad_directory.$image_name);   
            // insert process
            $sql = "INSERT INTO users(role_id,name,gender,phone,username,email,password,class_id,dept_id,designation, address,image,active) VALUES('$user_type','$name','$gender','$phone','$username','$email','$password','8','$dept','$design','$address','$image_name', '0')";      
            $user = $db->insert($sql);        
        ...
        if(!($user_type_error && $name_error && $gender_error && $phone_error && $username_error && $email_error && $password_error && $pass_mathc_error && $class_error && $roll_error && $dept_error && $phone_error && $address_error)) { 
            move_uploaded_file($tmp_name, $uplodad_directory.$image_name);   
            // insert process
            $sql = "INSERT INTO users(role_id, name, gender, phone, username, email, password, class_id, roll , dept_id, address, image, active) 
            VALUES('$user_type', '$name', '$gender', '$phone', '$username', '$email', '$password', '$class', '$roll', '$dept','$address', '$image_name', '0')";      
            $user = $db->insert($sql);    
          ...
          <div class="panel panel-default">
              ...
              <div class="panel-body">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                  ...
                  <button type="submit" name="user_register" class="btn btn-success btn-block btn-lg">Register</button>
